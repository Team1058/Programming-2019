...
public class VisionMPDriveToDonut extends Command{
    ...
    @Override
    public void init() {
        System.out.println("FollowMotionP init");
        //how far to stop before the vision target in inches
		
		//get x and convert to meters
		double x = Hardware.getInstance().limelight.getXYPosModified(RobotSpecs.BUBBLE_RADIUS)[0]/Constants.INCHES_IN_METERS;
		double y = Hardware.getInstance().limelight.getXYPosModified(RobotSpecs.BUBBLE_RADIUS)[1]/Constants.INCHES_IN_METERS;
